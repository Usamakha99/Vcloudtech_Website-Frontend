...
/** Partner logo marquee — optimized for light backgrounds. */
export function WhiteDesignTestSocialProofBar() {
          ...
          {marqueeLogos.map((logo, index) => (
            <li key={`${logo.name}-${index}`} className="flex shrink-0 items-center px-1 sm:px-2">
              <Image
                ...
                width={240}
                height={72}
                className="h-auto max-h-12 w-auto max-w-[10.5rem] object-contain opacity-60 grayscale transition duration-300 hover:scale-105 hover:opacity-100 hover:grayscale-0 sm:max-h-14 sm:max-w-[12.5rem] lg:max-h-16 lg:max-w-[14rem]"
                sizes="200px"
              />
            </li>
          ))}
